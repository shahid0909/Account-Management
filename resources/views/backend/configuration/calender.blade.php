@extends('backend.layout.master')
@section('content')

    <div class="px-3">
        <div class="container-fluid">

            <!-- Page Title -->
            <div class="py-3 py-lg-4">
                <div class="row">
                    <div class="col-lg-6">
{{--                        <h4 class="page-title mb-0">Calender Setup</h4>--}}
                    </div>
                    <div class="col-lg-6">
                        <ol class="breadcrumb m-0 float-end d-none d-lg-flex">
                            <li class="breadcrumb-item">Configuration</li>
                            <li class="breadcrumb-item active">Calender Setup</li>
                        </ol>
                    </div>
                </div>
            </div>

            <!-- Calendar Setup -->
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">

                            <!-- Header + Button -->
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h4 class="header-title mb-0">Calendar Setup</h4>

                                <button id="toggleBtn" class="btn btn-primary">
                                    <i id="iconPlus" class="mdi mdi-plus"></i>
                                    <i id="iconCheck" class="mdi mdi-close d-none"></i>
                                    <span id="btnText">Add New Setup</span>
                                </button>
                            </div>

                            <!-- Hidden Form -->
                            <div id="calendarForm" class="border rounded p-3 mb-4" style="display:none;">
                                <form>
                                    <div class="row">
                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Fiscal Period <span class="text-danger">*</span></label>
                                            <select class="form-select">
                                                <option>Select</option>
                                            </select>
                                        </div>

                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Fiscal Begin Year <span class="text-danger">*</span></label>
                                            <input type="number" class="form-control" placeholder="2024">
                                        </div>

                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Fiscal End Year <span class="text-danger">*</span></label>
                                            <input type="number" class="form-control" placeholder="2025">
                                        </div>

                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Fiscal Year</label>
                                            <input type="text" class="form-control" placeholder="2024-2025">
                                        </div>

                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Calendar Status</label>
                                            <select class="form-select">
                                                <option value="A">Active</option>
                                                <option value="I">Inactive</option>
                                            </select>
                                        </div>
                                    </div>

                                    <div class="text-end">
                                        <button type="button" class="btn btn-secondary" onclick="toggleForm()">Cancel</button>
                                        <button type="submit" class="btn btn-success">Save</button>
                                    </div>
                                </form>
                            </div>

                            <!-- Data Table -->
                            <table id="calender-table" class="table table-bordered dt-responsive nowrap w-100">
                                <thead class="table-light">
                                <tr>
                                    <th>Sl</th>
                                    <th>Fiscal Period</th>
                                    <th>Fiscal Year</th>
                                    <th>Calendar Status</th>
                                    <th>Action</th>
                                </tr>
                                </thead>
                                <tbody>
                                {{-- Dynamic Data --}}
                                </tbody>
                            </table>

                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>

    <!-- Toggle Script -->


@endsection

@section('script')

    <script>
        $('#toggleBtn, #cancelBtn').on('click', function () {

            $('#calendarForm').slideToggle(300);

            $('#iconPlus').toggleClass('d-none');
            $('#iconCheck').toggleClass('d-none');

            let text = $('#btnText').text();
            $('#btnText').text(text === 'Add New Setup' ? 'Close Form' : 'Add New Setup');

            $('#toggleBtn').toggleClass('btn-primary btn-success');
        });
        let table = $('#calender-table').DataTable({ // Change `.dataTable()` to `.DataTable()`
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('calender.datatable') }}",
                type: 'GET',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                }
            },
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: "fiscalPeriod"},
                {data: "fiscal_year"},
                {data: "status"},
                {data: 'action', name: 'action', orderable: false, searchable: false}
            ],


        });
    </script>
@endsection
